...
int main(int argc, char **argv) {
    ...
    if (debug_enabled) {
        ...
        if (!debug_log) {
            ...
            write_debug_banner(debug_log, "iterative-deepening", get_evaluation_profile(), depth, protocol_mode, debug_log_path);
        }
    }
    ...
    printf("Iterative Deepening Chess Bot (C)\n");
    printf("You are white. Enter UCI moves like e2e4. Type 'quit' to stop.\n");
    printf("Engine search max depth: %d\n", depth);
